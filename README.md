...
When `/gptprof try-codex` or `/gptprof use-pi` changes the default GPT route, the manager also updates explicit agent-level OpenAI/OpenAI-Codex overrides so topic agents do not stay pinned to an older provider/runtime. During profile switches, stale native `openai/*` session entries whose Codex app-server metadata is bound to another `authProfileId` are archived so the next turn starts a fresh thread under the selected profile.
...
Когда `/gptprof try-codex` или `/gptprof use-pi` меняет default GPT route, manager также обновляет явные agent-level OpenAI/OpenAI-Codex overrides, чтобы topic agents не оставались прибиты к старому provider/runtime. При смене профиля stale native `openai/*` sessions, где Codex app-server metadata привязана к другому `authProfileId`, архивируются; следующий turn стартует свежий thread уже под выбранным профилем.
